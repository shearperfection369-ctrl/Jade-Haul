...
export default function GpsMap({ load, stations = [], height = "100%" }) {
  if (!load) return null;
  const route = [
  ...
  return (
    <div className="relative w-full" style={{ height }} data-testid="gps-map">
      <MapContainer
        center={center}
        zoom={5}
      ...
      {["top-left", "top-right", "bottom-left", "bottom-right"].map((pos) => (
        <div key={pos} className={`absolute w-6 h-6 border-primary pointer-events-none ${
